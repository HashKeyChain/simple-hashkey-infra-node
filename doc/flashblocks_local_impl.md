...
> **The URL must end in `/ws`.** ws-proxy serves the stream only at `/ws` and has no route at `/`, so dropping the path returns 404
> on every connect. op-reth's Flashblocks client retries with no backoff, turning that 404 into a hot loop (~1600 attempts/sec,
> one ERROR line each) that saturates CPU and disk — enough starvation to stall op-rbuilder past its Engine API timeout and leave
> an unrecoverable gap in its chain. Base publishes the same shape: `wss://mainnet.flashblocks.base.org/ws`.
