...
echo "op-batcher --log.level=debug --l1-eth-rpc=$L1_RPC_URL --l2-eth-rpc=$L2_RPC_URL --rpc.port=$OP_BATCHER_PORT --rollup-rpc=http://$OP_NODE_RPC_URL --private-key=${GS_BATCHER_PRIVATE_KEY} --max-channel-duration=${BATCHER_MAX_CHANNEL_DURATION:-300} ${miscs}"
