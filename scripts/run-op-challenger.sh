...
if [ "$USE_FAULT_PROOFS" = "false" ]; then
  echo "Use fault proofs is disabled, skip starting op-challenger."
  exit 0
fi
...
base_flags="--log.level=debug --datadir=$BASE_PATH/data/op-challenger --l1-eth-rpc=$L1_RPC_URL --l1-beacon=$L1_RPC_URL --l2-eth-rpc=$L2_RPC_URL --rollup-rpc=$OP_NODE_RPC_URL --private-key=$private_key --trace-type=cannon --game-factory-address=$(cat $DEPLOYMENT_OUTFILE | jq -r .DisputeGameFactoryProxy)"
